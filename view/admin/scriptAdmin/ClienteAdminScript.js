function actualizar(id){
  ...
  $('#cargarVista').load('ClienteAdminController.php?accion=actualizar&id=' + id);
}

function eliminar(id,idusuario,foto) {
 Swal.fire({
  title: '¿Estas seguro?',
  text: "Quieres eliminar a " + idusuario +" del listado",
  icon: 'warning',
  ...
  confirmButtonText: '<i class="fas fa-check-circle"></i>' + ' Si, eliminar'
    ...
    if (result.value) {
      var url = 'ClienteAdminController.php?accion=eliminar&id='+id;
          location.href=url;
      Swal.fire(
        'Eliminado!',
...
function RegistrarCliente(){	
  ...
  $.post('ClienteAdminController.php','&'+$("#formulario").serialize(),function(respuesta){
    if (respuesta=="completado"){
      window.location.href="ClienteAdminController.php?accion=mostrar";
    }else{
      Swal.fire({
        position: 'top',
        icon: 'error',
